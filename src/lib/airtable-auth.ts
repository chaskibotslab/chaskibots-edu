...
export async function validateAccessCode(accessCode: string): Promise<{
  ...
  try {
    ...
    await base(USERS_TABLE).update(record.id, {
...
export async function deactivateUser(userId: string): Promise<boolean> {
  try {
    await base(USERS_TABLE).update(userId, { isActive: false })
    return true
  } catch (error) {
...
export async function regenerateAccessCode(userId: string): Promise<{
  ...
  try {
    ...
    await base(USERS_TABLE).update(userId, { accessCode: newCode })
...
export async function createCourse(
  ...
  try {
    const record = await base(COURSES_TABLE).create({
      name,
