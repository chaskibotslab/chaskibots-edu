      ...
      <header className="bg-gray-50 border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Gestión de Academia</h1>
              <p className="text-sm text-gray-600">Cursos, módulos y lecciones que consumen Python IDE, Terminal Linux y /academy</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={load} className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
              <RefreshCw className="w-4 h-4" /> Recargar
            </button>
            <button onClick={() => openCreate('course')} className="flex items-center gap-2 px-4 py-2 bg-brand-purple text-white rounded-lg font-medium hover:bg-brand-purple/90">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {courses.map(course => {
              ...
              return (
                <div key={course.id} className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                  <div className="flex items-center justify-between p-4">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{course.modules.length} módulos · {totalLessons} lecciones</p>
                      </div>
                      {expandedCourse === course.id ? <ChevronDown className="w-4 h-4 text-gray-400 ml-2" /> : <ChevronRight className="w-4 h-4 text-gray-400 ml-2" />}
                    </button>
                    <div className="flex items-center gap-2">
                      <button onClick={() => openCreate('module', course.id)} className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
                        <Plus className="w-3.5 h-3.5" /> Módulo
                      </button>
                      <button onClick={() => openEdit('course', course)} className="p-1.5 text-gray-500 hover:text-gray-900 hover:bg-gray-200 rounded-lg">
                  ...
                  {expandedCourse === course.id && (
                    <div className="border-t border-gray-200 divide-y divide-gray-200">
                      {course.modules.length === 0 && <p className="p-4 text-sm text-gray-400">Sin módulos todavía.</p>}
                        ...
                        <div key={mod.id} className="bg-white">
                          <div className="flex items-center justify-between p-3 pl-8">
                            ...
                            >
                              ...
                              {mod.level_id && <span className="text-[10px] px-1.5 py-0.5 bg-purple-100 text-purple-600 rounded-full">{mod.level_id}</span>}
                              <span className="text-xs text-gray-400">({mod.lessons.length} lecciones)</span>
                              {expandedModule === mod.id ? <ChevronDown className="w-3.5 h-3.5 text-gray-400" /> : <ChevronRight className="w-3.5 h-3.5 text-gray-400" />}
                            </button>
                            <div className="flex items-center gap-2">
                              <button onClick={() => openCreate('lesson', mod.id)} className="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
                                <Plus className="w-3 h-3" /> Lección
                              </button>
                              <button onClick={() => openEdit('module', mod)} className="p-1 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg">
                          ...
                          {expandedModule === mod.id && (
                            <div className="pl-14 pr-3 pb-3 space-y-1.5">
                              {mod.lessons.length === 0 && <p className="text-xs text-gray-400 py-1">Sin lecciones todavía — módulo vacío.</p>}
                              {mod.lessons.map(lesson => (
                                <div key={lesson.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg border border-gray-200">
                                  <div className="flex items-center gap-2 min-w-0">
                                    <FileText className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                                    <span className="text-xs text-gray-700 truncate">{lesson.title}</span>
                                    <span className="text-[10px] px-1.5 py-0.5 bg-gray-200 text-gray-600 rounded shrink-0">{lesson.difficulty}</span>
                                  </div>
                                  <div className="flex items-center gap-1 shrink-0">
                                    <button onClick={() => openEdit('lesson', lesson)} className="p-1 text-gray-500 hover:text-gray-900 hover:bg-gray-200 rounded">
                                      <Edit className="w-3.5 h-3.5" />
                                    </button>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-gray-200 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              ...
              <button onClick={closeModal} className="text-gray-500 hover:text-gray-900"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-4 space-y-4">
              ...
              {modal.entity === 'module' && (
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Nivel (level_id) — vacío = todos los grados asignados al curso</label>
                  <input
              ...
              {modal.entity === 'course' && (
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Icono (emoji)</label>
                    <input type="text" value={modal.data.icon} onChange={e => setModal({ ...modal, data: { ...modal.data, icon: e.target.value } })} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900" />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Color</label>
                    <input type="text" value={modal.data.color} onChange={e => setModal({ ...modal, data: { ...modal.data, color: e.target.value } })} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900" />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Dificultad</label>
                    <select value={modal.data.difficulty} onChange={e => setModal({ ...modal, data: { ...modal.data, difficulty: e.target.value } })} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
              ...
              {modal.entity === 'lesson' && (
                <>
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Teoría (markdown: #, ##, **negrita**, `código`, - listas)</label>
                    <textarea
                      value={modal.data.theory || ''}
                      onChange={e => setModal({ ...modal, data: { ...modal.data, theory: e.target.value } })}
                      rows={8}
                      className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 font-mono text-xs"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Ejemplos (JSON): [{'{'}"title","code","explanation"{'}'}]</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Retos (JSON): [{'{'}"title","description","starter_code","expected_output","hints":[]{'}'}]</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Minutos estimados</label>
                    <input
                      ...
                      className="w-32 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-gray-900"
                    />
                  </div>
                </>
              )}
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Orden (sort_order)</label>
                <input
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-12 h-6 rounded-full transition-all ${modal.data.is_active ? 'bg-brand-purple' : 'bg-gray-300'}`}
                >
                  <div className={`w-5 h-5 rounded-full bg-white transition-all ${modal.data.is_active ? 'ml-6' : 'ml-0.5'}`} />
                </button>
                <span className="text-sm text-gray-700">{modal.data.is_active ? 'Activo' : 'Inactivo'}</span>
              </div>
            </div>

            <div className="flex items-center justify-end gap-3 p-4 border-t border-gray-200">
              <button onClick={closeModal} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">Cancelar</button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex items-center gap-2 px-4 py-2 bg-brand-purple text-white rounded-lg font-medium hover:bg-brand-purple/90 disabled:opacity-50"
              >
