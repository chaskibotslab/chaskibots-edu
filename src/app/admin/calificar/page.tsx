    ...
    <div className="min-h-screen bg-slate-50 animate-fade-in">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/admin" className="w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-all active:scale-[0.98] shrink-0">
          ...
          {newCount > 0 && (
            <button
              ...
              className="flex items-center gap-2 bg-chaski-primary/10 text-chaski-primary font-semibold px-4 py-2 rounded-full text-sm animate-pulse transition-all active:scale-[0.98]"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:border-chaski-primary focus:ring-2 focus:ring-chaski-primary/10 transition-all"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-12 text-center animate-fade-in">
...
function SubmissionRow({ submission, task, onClick, style }: { submission: Submission; task?: Task; onClick: () => void; style?: React.CSSProperties }) {
  ...
  return (
    <button
      onClick={onClick}
      style={style}
      className="group w-full text-left bg-white rounded-3xl border border-slate-200 p-4 shadow-sm hover:shadow-lg hover:border-chaski-primary/40 transition-all active:scale-[0.99] animate-slide-up"
    >
      <div className="flex items-center gap-4">
        {/* Avatar */}
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold shrink-0 ${
          isGraded ? 'bg-gradient-to-br from-green-400 to-emerald-500' : 'bg-gradient-to-br from-chaski-primary to-chaski-accent'
        }`}>
          {initial}
        </div>
        ...
        <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-chaski-primary group-hover:translate-x-1 transition-all shrink-0" />
      </div>
    </button>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-2xl max-h-[94vh] bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden flex flex-col animate-slide-up">
        ...
        <div className="flex items-start justify-between gap-3 px-6 pt-4 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-chaski-primary to-chaski-accent flex items-center justify-center text-white font-bold shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center shrink-0 transition-all active:scale-[0.98]"
          >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                ...
                {submission.attachmentUrls.map((url, i) => {
                  ...
                  return (
                    <a
                      ...
                      className="group relative aspect-square rounded-2xl overflow-hidden border border-slate-200 bg-slate-50 hover:border-chaski-primary transition-all"
                    >
                      {isImg ? (
          ...
          {/* Grading panel */}
          <div className="bg-gradient-to-br from-chaski-primary/5 to-chaski-accent/5 border border-chaski-primary/20 rounded-3xl p-5 space-y-4">
            ...
            <div>
              <div className="flex items-baseline justify-between mb-2">
                ...
                <span className="text-3xl font-black text-chaski-primary">{grade}/100</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                className="w-full h-2 bg-slate-200 rounded-full appearance-none cursor-pointer accent-chaski-primary"
              />
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-chaski-primary" />
                Comentarios
              </label>
              <textarea
                value={feedback}
                ...
                className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:border-chaski-primary focus:ring-2 focus:ring-chaski-primary/10 transition-all resize-none"
              />
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {QUICK_FEEDBACK.map(fb => (
                  <button
                    ...
                    className="text-xs px-3 py-1.5 rounded-full bg-white text-slate-600 border border-slate-200 hover:border-chaski-primary hover:text-chaski-primary transition-all active:scale-[0.98]"
                  >
                    {fb}
                  </button>
                ))}
        ...
        <div className="border-t border-slate-100 px-6 py-4 bg-white flex gap-3">
          <button
            onClick={onClose}
            className="px-5 py-3 rounded-2xl text-slate-700 font-semibold hover:bg-slate-100 transition-all active:scale-[0.98]"
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-chaski-primary to-chaski-accent text-white font-bold py-3 rounded-2xl shadow-lg shadow-chaski-primary/30 transition-all disabled:opacity-50 active:scale-[0.98]"
          >
            {saving ? (
              <>
