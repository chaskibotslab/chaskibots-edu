...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      {/* Logo */}
      <div className={`flex items-center gap-2.5 px-4 h-14 border-b border-hack-green/15 ${collapsed ? 'justify-center px-2' : ''}`}>
        <div className="relative flex-shrink-0">
          <Image src="/chaski.png" alt="ChaskiBots" width={28} height={28} className="rounded-lg ring-1 ring-hack-green/30" />
          <span className="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-hack-green ring-2 ring-black animate-pulse" />
        </div>
        {!collapsed && (
          <div className="min-w-0 animate-fade-in">
            <p className="text-sm font-bold text-white truncate leading-tight font-mono">ChaskiBots</p>
            <p className="text-[10px] text-hack-green/60 leading-tight font-mono">root@admin:~#</p>
          </div>
        )}
      </div>
      ...
      {!collapsed && (
        <div className="px-3 pt-3">
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-hack-green/40" />
            <input
              value={query}
              onChange={e => setQuery(e.target.value)}
              placeholder="grep sección..."
              className="w-full pl-8 pr-3 py-1.5 text-xs font-mono rounded-lg bg-white/5 border border-hack-green/15 focus:border-hack-green/50 focus:bg-white/10 focus:outline-none transition-colors placeholder:text-white/30 text-white"
            />
          </div>
        </div>
      )}
                ...
                return (
                  <Link
                    ...
                    className={`group relative flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-medium transition-all duration-150 ${
                      ...
                      active
                        ? 'bg-hack-green/10 text-hack-green'
                        : 'text-white/60 hover:bg-white/5 hover:text-white'
                    }`}
                  >
                    {active && !collapsed && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-hack-green rounded-full shadow-hack-green" />
                    )}
                    <Icon className={`w-[18px] h-[18px] flex-shrink-0 transition-transform group-hover:scale-110 ${active ? 'text-hack-green' : 'text-white/30 group-hover:text-white/60'}`} />
                    {!collapsed && <span className="truncate">{item.label}</span>}
                    {!collapsed && active && (
                      <span className="ml-auto w-1.5 h-1.5 rounded-full bg-hack-green animate-pop" />
                    )}
                  </Link>
                )
              })}
      ...
      {/* Footer */}
      <div className="border-t border-hack-green/15 p-2 space-y-0.5">
        <Link
          href="/"
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex flex-col bg-chaski-dark border-r border-hack-green/15 sticky top-0 h-screen transition-[width] duration-200 ease-out relative overflow-hidden ${
          collapsed ? 'w-16' : 'w-60'
        }`}
      >
        <div className="absolute inset-0 opacity-[0.05] pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(57,255,20,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(57,255,20,0.5) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
        <div className="relative flex flex-col h-full">{sidebarContent}</div>
        <button
          onClick={toggleCollapsed}
          className="absolute -right-3 top-16 w-6 h-6 rounded-full bg-chaski-dark border border-hack-green/30 shadow-sm flex items-center justify-center text-hack-green/60 hover:text-hack-green hover:border-hack-green/60 transition-colors z-10"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topbar */}
        <header className="sticky top-0 z-40 h-14 bg-chaski-dark border-b border-hack-green/15 flex items-center gap-3 px-4">
          <button
          ...
          <nav className="flex items-center gap-1.5 text-[13px] min-w-0 font-mono">
            ...
            {breadcrumbs.map((crumb, i) => (
              <span key={crumb.href} className="flex items-center gap-1.5 min-w-0">
                ...
                {crumb.isLast ? (
                  <span className="font-semibold text-hack-green truncate">{crumb.label}</span>
                ) : (
            ...
            <span className="w-1.5 h-3.5 bg-hack-green/70 animate-cursor-blink ml-0.5" />
          </nav>

          <div className="ml-auto flex items-center gap-3">
            {user && (
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-chaski-primary to-chaski-accent flex items-center justify-center text-white text-xs font-bold ring-1 ring-hack-green/30">
                  {(user.name || user.email || '?').charAt(0).toUpperCase()}
                </div>
                <div className="hidden sm:block leading-tight">
                  <p className="text-xs font-semibold text-white">{user.name || user.email}</p>
                  <p className="text-[10px] text-hack-green/60 capitalize font-mono">{user.role}</p>
                </div>
              </div>
            )}
          </div>
